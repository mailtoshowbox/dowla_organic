...
class ControllerInstallStep3 extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$a1 = DIR_STORAGE . 'modification/';
			setcookie('DIR_MODIFICATION', $a1 , time() + (86400 * 30), "/");

			$a2 = addslashes(DIR_OPENCART) . 'system/';
			setcookie('DIR_SYSTEM', $a2 , time() + (86400 * 30), "/");

			$a3 = addslashes(DIR_OPENCART) . 'catalog/';
			setcookie('DIR_CATALOG', $a3 , time() + (86400 * 30), "/");

			$a4 = addslashes(DIR_OPENCART) . 'admin/';
			setcookie('DIR_APPLICATION', $a4 , time() + (86400 * 30), "/");
