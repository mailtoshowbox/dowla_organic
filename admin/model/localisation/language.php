...
class ModelLocalisationLanguage extends Model {
	...
	public function addLanguage($data) {
		...
		$oldlanguage_id = $this->config->get('config_language_id');

		/*brand*/
		$name		 				= "tntbrandlist";
		$detail		 				= $this->getcommonmoduledetail($name);
		$commonmoduledetail   		= json_decode($detail['setting'],1);
		$settingdata            	= array();
        $settingdata['name']    	= $commonmoduledetail['name'];
        $settingdata['status']  	= $commonmoduledetail['status'];
		foreach ($commonmoduledetail['tntbrandlist_parent'] as $value) {
            $settingdata['tntbrandlist_parent'][$oldlanguage_id] =  array('title'=>$value['title'],'subtitle'=>$value['subtitle']);
            $settingdata['tntbrandlist_parent'][$language_id] =  array('title'=>$value['title'],'subtitle'=>$value['subtitle']);

        }
        $this->editModule($detail['module_id'], $settingdata);

		$query 			= $this->db->query("SELECT * FROM " . DB_PREFIX . "tntbrandlist ORDER BY tntbrandlist_position");
		$text 		= array();
		foreach ($query->rows as $brand) {
			$text = json_decode($brand['tntbrandlist_text'],1);
			$text[(int)$oldlanguage_id] = array('title' => $text[$oldlanguage_id]['title']);
			$text[(int)$language_id]  = array('title' => $text[$oldlanguage_id]['title']);

			$this->db->query("UPDATE " . DB_PREFIX . "tntbrandlist SET tntbrandlist_text = '" . json_encode($text) . "' WHERE tntbrandlist_id = '" . (int)$brand['tntbrandlist_id'] . "'");
		}
		/*brand*/

		/*category slider*/
		/*$name		 				= "tntcategoryslider";
		$detail		 				= $this->getcommonmoduledetail($name);
		$commonmoduledetail   		= json_decode($detail['setting'],1);
		$settingdata            	= array();
        $settingdata['name']    	= $commonmoduledetail['name'];
        $settingdata['status']  	= $commonmoduledetail['status'];
		foreach ($commonmoduledetail['tntcategoryslider_parent'] as $value) {
            $settingdata['tntcategoryslider_parent'][$oldlanguage_id] =  array('title'=>$value['title'],'subtitle'=>$value['subtitle']);
            $settingdata['tntcategoryslider_parent'][$language_id] =  array('title'=>$value['title'],'subtitle'=>$value['subtitle']);
        }
        $this->editModule($detail['module_id'], $settingdata);

		$query 			= $this->db->query("SELECT * FROM " . DB_PREFIX . "tntcategorysliderchild");
		$text 		= array();
